...
/**
   ...
   @param new_extn a new file extension for \a file.  \a new_extn
     should not include a dot (i.e. \c "jpg", not \c ".jpg") unless the new
     file extension should contain two dots.
   @return Returns \a file with its extension replaced.  If
     \a file does not have an extension, this function simply
     adds one.
*/
...
/**
   ...
   @return Returns a full path name with \a path prepended to
     \a file.
*/
...
/**
   ...
   @return Returns \a pathname with everything up to and including
     the last \c '/' removed.  If \a pathname does not contain a \c '/',
     it is returned without modification.
*/
extern char* remove_path( const char* pathname );


/**
   A function to determine whether a file is an image file.  The determination
   is made based solely on the file's extension.  Files with the following
   extensions are considered image files: .\c png, .\c jpg, .\c jpeg, .\c pbm,
   .\c pgm, .\c ppm, .\c bmp, .\c tif, .\c tiff.

   @param file a file name
   @return Returns TRUE if \a file has one of the extensions listed
...
/**
   ...
   Every time this function is called, the state of the pinwheel is
   incremented.  The pinwheel has four states that loop indefinitely: | / - \.
   
   @param done if FALSE, this function simply increments the state of the
     pinwheel; if TRUE, prints "done" rather than incrementing the pinwheel.
*/
